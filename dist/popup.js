...
// supabase-config.js
var supabaseUrl = "https://djtriaizzxfknrfsogbs.supabase.co";
var supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRqdHJpYWl6enhma25yZnNvZ2JzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDcyMjcyMTMsImV4cCI6MjA2MjgwMzIxM30.L80d5pvNZ_fCZF9WucvgvUIK729wgleX9m0I5buR84w";
var supabase = createClient(supabaseUrl, supabaseKey);
var supabase_config_default = supabase;

// auth.js
var AuthService = class {
  async signInWithGoogle() {
    try {
      const { data, error } = await supabase_config_default.auth.signInWithOAuth({
        provider: "google",
        options: {
  ...
  async signOut() {
    try {
      const { error } = await supabase_config_default.auth.signOut();
      if (error) throw error;
    } catch (error) {
  ...
  async getCurrentUser() {
    try {
      const { data: { user }, error } = await supabase_config_default.auth.getUser();
      if (error) throw error;
      return user;
    } catch (error) {
  ...
  onAuthStateChange(callback) {
    return supabase_config_default.auth.onAuthStateChange((event, session) => {
...
var ApplicationsService = class {
  async recordApplication(application) {
    try {
      const { user } = await supabase_config_default.auth.getUser();
      if (!user) {
        throw new Error("No authenticated user");
      }
      const { data, error } = await supabase_config_default.from("applications").insert({
        user_id: user.id,
  ...
  async getApplications(filters = {}) {
    try {
      ...
      let query = supabase_config_default.from("applications").select("*").eq("user_id", user.id).order("application_date", { ascending: false });
      ...
      if (filters.needsFollowUp) {
        query = query.not("follow_up_date", "is", null).lte("follow_up_date", (/* @__PURE__ */ new Date()).toISOString());
      }
  ...
  async updateApplication(id, updates) {
    try {
      ...
      const { data, error } = await supabase_config_default.from("applications").update({
        ...updates,
  ...
  async deleteApplication(id) {
    try {
      ...
      const { error } = await supabase_config_default.from("applications").delete().eq("id", id).eq("user_id", user.id);
      if (error) throw error;
      return true;
    } catch (error) {
  ...
  async getApplicationStats() {
    try {
      ...
      const { data, error } = await supabase_config_default.from("applications").select("status").eq("user_id", user.id);
      if (error) throw error;
      const stats = {
        total: data.length,
        byStatus: {},
  ...
  async getUpcomingInterviews() {
    try {
      ...
      const { data, error } = await supabase_config_default.from("applications").select("*").eq("user_id", user.id).not("interview_date", "is", null).gte("interview_date", (/* @__PURE__ */ new Date()).toISOString()).order("interview_date", { ascending: true });
      if (error) throw error;
      return data;
    } catch (error) {
  ...
  async getFollowUpsNeeded() {
    try {
      ...
      const { data, error } = await supabase_config_default.from("applications").select("*").eq("user_id", user.id).not("follow_up_date", "is", null).lte("follow_up_date", (/* @__PURE__ */ new Date()).toISOString()).order("follow_up_date", { ascending: true });
      if (error) throw error;
      return data;
    } catch (error) {
...
async function initializeAuth() {
  const user = await auth_default.getCurrentUser();
  ...
  auth_default.onAuthStateChange((event, session) => {
    if (event === "SIGNED_IN") {
      authContainer.style.display = "none";
      appContainer.style.display = "block";
      if (session?.user) {
...
function initializeEventListeners() {
  document.getElementById("sign-in").addEventListener("click", async () => {
    try {
      await auth_default.signInWithGoogle();
    } catch (error) {
...
async function handleFillIt() {
  try {
    ...
    await applications_default.recordApplication(application);
...
async function loadApplications(filters = {}) {
  try {
    const applications = await applications_default.getApplications(filters);
    ...
    document.querySelectorAll(".status-select").forEach((select) => {
      select.addEventListener("change", async (e) => {
        ...
        try {
          await applications_default.updateApplication(id, { status: newStatus });
    ...
    document.querySelectorAll(".btn-delete").forEach((button) => {
      button.addEventListener("click", async (e) => {
        ...
        if (confirm("Are you sure you want to delete this application?")) {
          try {
            await applications_default.deleteApplication(id);
  ...
  input.onchange = async (e) => {
    ...
    try {
      ...
      Object.keys(data.profile).forEach((key) => {
        ...
        if (input2) input2.value = data.profile[key];
      });
...
function scheduleSync() {
  if (document.getElementById("auto-sync").checked) {
    ...
    syncTimeout = setTimeout(saveProfile, 1e3);
  }
}
...
async function showApplicationStats() {
  try {
    const stats = await applications_default.getApplicationStats();
...
async function showUpcomingInterviews() {
  try {
    const interviews = await applications_default.getUpcomingInterviews();
...
async function showFollowUpsNeeded() {
  try {
    const followUps = await applications_default.getFollowUpsNeeded();
    ...
    document.querySelectorAll(".btn-followup").forEach((button) => {
      button.addEventListener("click", async (e) => {
        const id = e.target.dataset.id;
        try {
          await applications_default.updateApplication(id, { follow_up_date: null });
...
function showEditModal(application) {
  ...
  modal.querySelector("form").addEventListener("submit", async (e) => {
    ...
    try {
      await applications_default.updateApplication(application.id, updates);
